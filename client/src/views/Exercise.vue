<template>
  <div>
    ...
    <p v-if="user.username == 'guest'">
      If you have not added more, the guest database has 300 hundred entries, 100 for each word type.
      The correct answer is the number next to the word.Ex: noun44 = 44
    </p>
    ...
    <div>
      ...
      <div v-if="exerciseDone">
        ...
        <div v-if="!allCorrect">
          ...
          <div v-for="(data, index) in wrongAnswers" :key="index">{{data.german}} - {{data.english}}</div>
        </div>
        <button @click="exerciseDone = !exerciseDone">Got It</button>
      </div>
      ...
      <div v-if="exerciseStarted">
        <div class="question" v-for="(question, index) in exerciseQuestions" :key="question.id">
          ...
          <input
            type="text"
            placeholder="Deutsch Übersetzung"
            v-model="userAnswers[index]"
            :class="{correct : userAnswers[index] == question.german}"
          />
        </div>
      </div>
  ...
  methods: {
      ...
      if (this.exerciseData.type == "random") {
        if (savedWordsLength < this.exerciseData.amount) {
          (this.inputError = true),
            (this.message =
              "You want to practice more words than you have in your database. Want to add more words? Currently you have" +
              savedWordsLength +
              " words in your DB.");
        } else {
    ...
    checkAnswers() {
      ...
      if (this.allCorrect == true) {
        ...
        this.userScore =
          this.exerciseQuestions.length - this.wrongAnswers.length;
      }
    }
  },
  computed: {
